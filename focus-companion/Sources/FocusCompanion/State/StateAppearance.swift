...
extension StateAppearance {
    ...
    static let table: [CompanionState: StateAppearance] = [
        ...
        // Логику "сжигания" задачи достроила Фаза 4б (TaskPanelController —
        // фоновое состояние, пока горит хотя бы одна задача). Здесь по-
        // прежнему только внешний вид по ориентирам из PHASE-3-PROMPT.md
        // ("сузились", "тёплая подсветка"); позиционирование подсветки
        // именно СНИЗУ капсулы (а не сверху, как у остальных состояний) в
        // 4б не делали — PHASE-4B-PROMPT.md этого явно не требовал, а
        // общий `highlightGlow` в `NotchRootView` один на все состояния.
